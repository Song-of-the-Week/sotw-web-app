...
class CRUDSong(CRUDBase[Song, SongCreate, SongUpdate]):
    def get_song_from_user(self, session: Session, *, submitter_id: int, week_id: int) -> Song:
        """
        ...
        """
        return session.query(Song).filter(and_(Song.submitter_id == submitter_id, Song.week_id == week_id)).first()
