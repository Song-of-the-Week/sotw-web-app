  ...
  <div class="container">
    <div class="row">
      <div class="col">
        ...
        <h2 v-if="firstPlace.length === 2">Tied for first place with {{ firstPlaceVotes }} votes each, we have {{ firstPlace[0] }} and {{ firstPlace[1] }}!</h2>
        <h2 v-else-if="firstPlace.length > 2">
          Tied for first place with {{ firstPlaceVotes }} votes each, we have
          <span v-for="(song, index) in firstPlace">{{ song }}<span v-if="index === firstPlace.length - 1">!</span><span v-else-if="index === firstPlace.length - 2">, and </span><span v-else>, </span></span>
        </h2>
        <div v-else>
          <h2 class="mb-4">In first place with {{ firstPlaceVotes }} votes, we have {{ firstPlace[0] }}!</h2>
          <h3 v-if="secondPlace.length === 2">Tied for second place with {{ secondPlaceVotes }} votes each, we have {{ secondPlace[0] }} and {{ secondPlace[1] }}!</h3>
          <h3 v-else-if="secondPlace.length > 2">
            Tied for second place with {{ secondPlaceVotes }} votes each, we have
            <span v-for="(song, index) in secondPlace">{{ song }}<span v-if="index === secondPlace.length - 1">!</span><span v-else-if="index === secondPlace.length - 2">, and </span><span v-else>, </span></span>
          </h3>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="guesser in guessingData">
                ...
                <td v-for="guess in guesser.guesses" :class="{ 'correct-guess': guess.submitterGuess === guess.submitterReal }">{{ guess.submitterGuess }}</td>
              </tr>
            </tbody>
          </table>
        </div>
